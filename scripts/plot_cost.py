#!/usr/bin/env python
"""Plot eval_cost_ema vs step for runs under a logdir."""
import argparse
import json
import pathlib
...
def read_eval_cost(metrics_path: pathlib.Path) -> Tuple[List[int], List[float]]:
    steps, costs = [], []
    ...
    with metrics_path.open() as f:
        for line in f:
            ...
            if "eval_episode/cost_ema" in rec:
                steps.append(rec.get("step", 0))
                costs.append(rec["eval_episode/cost_ema"])
    return steps, costs
...
def plot(logdir: pathlib.Path, outdir: pathlib.Path, per_run: bool) -> None:
    ...
    if per_run:
        for run in runs:
            steps, costs = read_eval_cost(run / "metrics.jsonl")
            if not steps:
                continue
            plt.figure(figsize=(6, 4))
            plt.plot(steps, costs, label=run.name)
            plt.xlabel("step")
            plt.ylabel("eval_cost_ema")
            plt.title(run.name)
            plt.grid(True, alpha=0.3)
            plt.legend()
            plt.tight_layout()
            plt.savefig(outdir / f"cost_{run.name}.png", dpi=200)
            plt.close()
    else:
        ...
        plt.xlabel("step")
        plt.ylabel("eval_cost_ema")
        plt.title(f"Eval cost ema vs step ({logdir.name})")
        plt.grid(True, alpha=0.3)
        plt.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(outdir / "cost.png", dpi=200)
        plt.close()


def main() -> None:
    parser = argparse.ArgumentParser(description="Plot eval_cost_ema vs step.")
